...
class CRM_OdooContributionSync_CreditInvoice {
  ...
  public function credit($odoo_invoice_id, DateTime $date, $contribution, $contribution_id=false) {
    ...
    $refund_invoice_id = $this->createCreditInvoice($invoice, $date, $contribution, $contribution_id=false);
  ...
  protected function convertInvoiceLineToCreditInvoiceLine($invoice, $refund_invoice_id) {
    ...
    foreach ($invoice['invoice_line']->scalarval() as $invoice_line_res) {
      ...
      if ($invoice_line['account_id']->scalarval()) {
        $line['account_id'] = new xmlrpcval(reset($invoice_line['account_id']->scalarval()), 'int');
      }
